...
import {getCategoryById, getProfessionalsByCategoryId, listCategories} from "../sql/category.js";
...
export const adminGetProfessionalsByCategory = async (req: Request, res: Response, next: NextFunction) => {
  const {user} = res.locals;
  if (!isAdmin(user)) return noAccess(res);
  const {id} = req.params;

  try {
    const result: any = await getProfessionalsByCategoryId(id, false);
    return success(res, {data: result, count: result.length});
  } catch (e) {
    return failure(res, e);
  }
}
